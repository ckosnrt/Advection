...
from BasicSchemes import BasicScheme

def animate(nt, ax, x, dx, nx, dt, a, b, c, u, ICnumber):
    ax.clear()
    result, analytic, initial = BasicScheme(x, nx, dx, nt, dt, a, b, c, u, ICnumber)
    ...
    plt.title(u"Advection for {} timesteps".format(nt))
    plt.xlabel('X')
    plt.ylabel('$\Phi$')
    plt.xlim([0,1])
    plt.ylim([-0.4,1.5])
...
def gifmaker(nt, ICnumber):
    ...
    ani = FuncAnimation(fig, animate, blit=True, interval=40, repeat=True,
                        frames=nt, fargs=(ax, x, dx, nx, dt, a, b, c, u, ICnumber,))    
